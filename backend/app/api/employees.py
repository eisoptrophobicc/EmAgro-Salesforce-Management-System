...
def create_employee(request: CreateEmployeeRequest, db: Session = Depends(get_db), current_sub_admin: User = Depends(get_current_sub_admin),):
    employee = EmployeeService.create_employee(db, request, current_sub_admin,)
