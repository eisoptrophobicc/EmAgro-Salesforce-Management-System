...
from app.exceptions.user import (AttendanceAlreadyMarkedError, AttendanceNotFoundError, EmailAlreadyExistsError, EmployeeAlreadyExistsError, EmployeeNotFoundError, RoleNotFoundError, TaskAlreadyExistsError, TaskNotFoundError, UserNotFoundError, DailyActivityAlreadyExistsError, DailyActivityNotFoundError,)
...
def register_exception_handlers(app: FastAPI):
    ...
    async def daily_activity_already_exists_handler(request: Request, exc: DailyActivityAlreadyExistsError,):
        return JSONResponse(status_code=409, content={"success": False, "message": str(exc),},)

    @app.exception_handler(DailyActivityNotFoundError)
    async def daily_activity_not_found_handler(request: Request, exc: DailyActivityNotFoundError,):
        return JSONResponse(status_code=404, content={"success": False, "message": str(exc),},)